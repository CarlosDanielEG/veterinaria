	...
	class ventaControlador extends ventaModelo{
		...
		*/
		public function guardar_venta_controlador(){
			...
			if (session_start(['name' => 'VETP'])) {
				if (!isset($_SESSION['id_vetp'])) {
					$alerta_simple = [
						"Alerta" => "warning",
						"Titulo" => "Ocurrió un error inesperado",
						"Texto" => "Fallo a iniciar variable de sesión"
					];
					echo json_encode($alerta_simple);  // Cambiar $alerta por $alerta_simple
					exit();
				} else {
					$token = md5($_SESSION['id_vetp']);
...
if ($result > 0) {
    ...
    if ($guardar_venta->rowCount() > 0) {
        ...
        $alerta_simple = [
            ...
            "Texto" => "La venta fue procesada con éxito",
            "Data" => $data
        ];
    } else {
        $alerta_simple = [
    ...
    $alerta_simple = [ // Cambiar $alerta por $alerta_simple
        "Alerta" => "warning",
        "Titulo" => "Ocurrió un error inesperado",
