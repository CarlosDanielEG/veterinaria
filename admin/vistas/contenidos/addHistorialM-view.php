    ...
    <div class="intro mb-4 add-historial-mct">
      <form action="<?php echo SERVERURL; ?>ajax/historialAjax.php" data-form="save" method="POST" class="FormularioAjax" enctype="multipart/form-data">
        ...
        <ul class="nav nav-tabs py-2 mb-2">
          ...
          <li class="nav-item ml-4">
            <a class="sub-titulo-panel nav-link active" data-toggle="modal"
              data-target="#modalReceta" href="#p">
          ...
          <div class="row tab-pane active" id="panel1_historia">
            <div class="col-lg-12">
              <div class="row">
                ...
                <div class="col-12">
                  <div class="group">
                    <input type="text" pattern="[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,#\- ]{1,100}" name="historia_motivo_reg" id="historia_motivo_reg" <?php if ($motivo_cita != "") {
                                                                                                                                              echo $value_motivo;
                                                                                                                                            } ?> required="" />
...
<div id="modalReceta" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="my-modal-title" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" value="<?= $campos_cita['idCita']; ?>" id="idCita">
        <input type="hidden" value="<?= $campos['idmascota']; ?>" id="idmascota">
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              <label for="medicamento">Medicamento</label>
              <input type="text" class="form-control" id="medicamento" placeholder="Ingrese el nombre del medicamento" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="dosis">Dosis</label>
              <input type="text" class="form-control" id="dosis" placeholder="Ingrese la dosis recomendada" required>
            </div>
          </div>
          <div class="col-md-12">
            <div class="form-group">
              <label for="instrucciones">Instrucciones</label>
              <textarea class="form-control" id="instrucciones" rows="4" placeholder="Escriba las instrucciones de uso" required></textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <script>
        function guardarReceta() {
          ...
          $.ajax({
            url: '<?php echo SERVERURL; ?>ajax/guardar_receta.php',
            method: 'POST',
            data: {
              medicamento,
              dosis,
            ...
            success: function(response) {
              ...
              if (res.Tipo == 'success') {
                setTimeout(() => {
                  window.open('<?php echo SERVERURL; ?>ajax/guardar_receta.php?generarReporte=' + res.ID);
